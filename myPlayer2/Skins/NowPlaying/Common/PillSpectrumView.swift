//
//  PillSpectrumView.swift
//  myPlayer2
//
//  Shared fullscreen/window pill spectrum visualizer for skins.
//

import AppKit
import SwiftUI

struct NowPlayingPillSpectrumView: View {
    ...
    var body: some View {
        NowPlayingPillSpectrumContainer(
...
private struct NowPlayingPillSpectrumContainer: NSViewRepresentable {
    let isPlaying: Bool
    let isDark: Bool
    ...
    func makeNSView(context: Context) -> NowPlayingPillSpectrumHostView {
        let view = NowPlayingPillSpectrumHostView(spectrumProvider: spectrumProvider)
    ...
    func updateNSView(_ nsView: NowPlayingPillSpectrumHostView, context: Context) {
    ...
    static func dismantleNSView(_ nsView: NowPlayingPillSpectrumHostView, coordinator: ()) {
        nsView.stop()
    }
}

@MainActor
private final class NowPlayingPillSpectrumHostView: NSView {
    ...
    private func layoutCapsules() {
        ...
        let totalBarsWidth = CGFloat(9) * capsuleWidth + CGFloat(8) * capsuleSpacing
        let originX = (width - totalBarsWidth) * 0.5
        let centerY = height * 0.5
        let minHeight = capsuleWidth
        let maxHeight = height * 0.92
        ...
        CATransaction.setDisableActions(true)
        for index in 0..<9 {
            let amplitude = CGFloat(currentWave[index])
            let barHeight = minHeight + (maxHeight - minHeight) * amplitude
            let x = originX + CGFloat(index) * (capsuleWidth + capsuleSpacing)
            let y = centerY - barHeight * 0.5

            let capsuleFrame = CGRect(x: x, y: y, width: capsuleWidth, height: barHeight)

            let layer = capsuleLayers[index]
            layer.frame = capsuleFrame
            layer.cornerRadius = capsuleWidth * 0.5

            let strokeLayer = strokeLayers[index]
            strokeLayer.frame = capsuleFrame
            strokeLayer.path = CGPath(
                roundedRect: capsuleFrame.insetBy(dx: 0.25, dy: 0.25),
                cornerWidth: max(0, capsuleWidth * 0.5 - 0.25),
                cornerHeight: max(0, capsuleWidth * 0.5 - 0.25),
                transform: nil
            )
        }
        CATransaction.commit()
    ...
    private static func makeCapsuleColorsWithStroke(
        ...
        let fills: [CGColor] = (0..<9).map { index in
        ...
        let strokes: [CGColor] = (0..<9).map { index in
            let t = CGFloat(index) / CGFloat(total)
            return makeStrokeColor(
                leftBase: leftBase,
                rightBase: rightBase,
                t: t,
                isDark: isDark
    ...
    private static func makeInterpolatedColor(
        ...
        if isDark {
            targetBrightness = max(0.18, min(0.32, brightness * 0.55))
            targetAlpha = 0.85
            saturation *= 0.92
        } else {
            targetBrightness = min(max(0.20, brightness * 0.78), 0.65)
            targetAlpha = 0.88
        }
    ...
    private static func makeStrokeColor(
        leftBase: NSColor,
        rightBase: NSColor,
        t: CGFloat,
        isDark: Bool
    ) -> NSColor {
        let base = makeInterpolatedColor(
            leftBase: leftBase,
            rightBase: rightBase,
            t: t,
            isDark: isDark
        ...
        let brightness = min(1, base.brightnessComponent + (isDark ? 0.18 : 0.12))

        return NSColor(
            calibratedHue: base.hueComponent,
            saturation: max(0.05, base.saturationComponent * 0.82),
