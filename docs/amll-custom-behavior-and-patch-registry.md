...
| Artistic fullscreen lyrics Tone Ladder | 艺术背景类 fullscreen lyrics 用不透明 OKLCH tone ladder 表达 active / inactive / secondary 层级，避免回到单纯 opacity。 | Swift `PerceptualToneLadder` + `SemanticPaletteFactory.fullscreenLyricsColorSet(... usesArtisticBackground:)` | 仅在 Swift 侧 `settings.fullscreenArtBackgroundEnabled` 路径启用；Web 继续只消费显式颜色与渲染结构，不重新决定 hue。Apple / Cover Gradient / Cover Blur profile 保持原 lighter/darker blend 语言。nearMono visible lyrics chroma 仍 ≤ 0.005。 |
...
- Phase 6 后，艺术背景类 fullscreen lyrics 可在 Swift 侧通过 `PerceptualToneLadder` 深化 `LyricsSurfaceColorSet`；这仍属于 Swift-owned color decision，不改变 Web rendering-only 边界。
