...
| Artistic fullscreen lyrics Tone Ladder (v2 single-seed) | 艺术背景类 fullscreen lyrics 用不透明 OKLCH tone ladder 表达 active / inactive / secondary 层级。v2 改为 single-seed：所有角色从同一个 lyric semantic seed 派生，inactive chromaScale 不机械低于 active；背景色仅作可读性校准，绝不作为 lyric 颜色种子。 | Swift `PerceptualToneLadder` + `SemanticPaletteFactory.fullscreenLyricsColorSet(... usesArtisticBackground:)` + `FullscreenPlayerView.resolveFullscreenLyricsInactiveBaseColor(...)` | 仅在 Swift 侧 `settings.fullscreenArtBackgroundEnabled` 路径启用；Web 继续只消费显式颜色与渲染结构，不重新决定 hue。`resolveFullscreenLyricsInactiveBaseColor(...)` 不允许引用 BKArt surface background / `primaryBackgroundColor` / `lockedFullscreenLyricsBackgroundColor` 三条旧路径（v1 灰白化根因）。inactive C ≥ active C × 0.85，hueΔ vs seed ≤ 0.025。Apple / Cover Gradient / Cover Blur profile 保持原 lighter/darker blend 语言。nearMono visible lyrics chroma 仍 ≤ 0.005。 |
