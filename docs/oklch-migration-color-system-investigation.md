...
报告共 13 节。`A`–`G` 是系统建模；`H` 是 R2 的 8 项定点核查结果；`I` 给出 R2 后的迁移路线结论；`J` 是 R3 的艺术决策引擎深度逆向；`K`–`R` 是 R3 加入的"颜色系统 2.0"方向性结论与对原结论的修订；`S` 是 R3 改版后总结的迁移路线最终纲领；`T` 是 R3 changelog。
...
> **注**：I 节是 R2 完成时的迁移路线。R3 在 J–S 中对其中"小面积高彩、Ultra Dark、忠实度、Spectrum / Home Shapes / 歌词决策位置"等做了关键修正与方向扩展，请以 S 节为最终纲领。
...
### J.1 小面积高显著色机制：已明显弱化（修正 R1 的"系统大概仍保留"猜测）
...
| 场景 | bucket area | 是否进入 topPalette | 是否进入 richPalette | 是否影响 globalAccent |
|---|---|---|---|---|
| 95% 黑灰 + 5% 鲜黄 | 0.05 | ✗（被 isNearMono L298 过滤） | ✗（同样被噪声门槛压下） | ✗ |
| 90% 深蓝绿 + 10% 明橙 | 0.10 | ✓ | ✓ | ✗（dominant 仍是蓝绿） |
| 80% 低饱暗色 + 20% 红 | 0.20 | 取决于 avgSat | 取决于 avgSat | ✗ |

**结论**：
- 当前小面积高显著色机制 **已明显弱化**，仅 `richPalette` 第二轮补救留存了"封面真实多色"采样能力。
- 但**消费端为 0**：没有任何 UI 直接读 richPalette；`optimizedAccent` 死守 dominantColor。
- **R1 中"系统大概仍保留小面积高显著色"的判断需要修正为"采样保留 / 派色完全丢失"**。
- 这是后续 ☆ K.1（频谱真实多色）、☆ K.3（Home Shapes 真实多色）的代码事实基础。

### J.2 Ultra Dark / 极暗封面保真机制：仅在歌词局部，主链路无系统级路径

**代码事实**：

| 路径 | 字面量是否存在 | 是否进入 SemanticPaletteFactory 主链路 | 输入指标 | 影响目标 |
|---|---|---|---|---|
| `BKArtBackgroundView.swift:296-300` `isUltraDarkPalette` | ✓ | ✗ | `imageCoverLuma`、`areaDominantB`、`grayScore`（多指标 OR）| BKArtBackgroundController `isUltraDarkActive` |
| `BKArtBackgroundView.swift:1107-1111` `updateUltraDarkOverlay` | ✓ | ✗ | `imageCoverLuma < 0.22` 独立阈值 | 50% 黑色覆盖蒙版（仅 BKArt 视觉层） |
| `FullscreenPlayerView.swift:3143, 3462-3471` | ✓ `lockedFullscreenLyricsUltraDark` | ✗ | `bkController.isUltraDarkActive` | 歌词 HSL L 二次下压 0.17-0.22、活跃下压 0.06-0.10、饱和度收 0.34-0.40 |
| `SemanticPalette.swift:95-96 → 186-225` `nearMonochromeAccent` | ✗（隐性） | ✓ | `analysis.isEffectivelyMonochrome` | globalAccent / uiAccent 强行落到 L ∈ [0.66, 0.74]（深）或 [0.32, 0.42]（浅） |

**结论**：
- **系统级真正承担"极暗保护"功能的是 `nearMonochromeAccent`**，但它的副作用是把所有极暗封面的 accent 提到 L≥0.66，**牺牲了"压暗、沉静、夜色感"**。
- 字面量 `ultraDark` 只作用于 BKArtBackground 视觉层 + Fullscreen 歌词的二次 HSL 修正，**不进入 SemanticPaletteFactory / HomeAmbientPalette / Cover Blur profile**。
- 当前 **缺一个"极暗封面允许 accent 仍偏暗 / 仍保留原色相"的路径**。这是 ☆ K.7（Artwork Readability Profile 统一化）的核心需求之一。
...
详见 ☆ R.1：迁移期 token 化时必须先做这 9 条规则的 OKLCH 等价校准。
...
### ☆ K.2 极暗封面应允许 accent 保持暗调（系统级 Ultra Dark）

**当前状态**：J.2 中详述，nearMonochromeAccent 把极暗封面的 accent 强提到 L≥0.66。

**方向性结论**：
1. 在 `SemanticPaletteFactory` 主链路引入"系统级 Ultra Dark"判断，输入指标统一为 WCAG relativeLuminance（避免 HSL/HSB 之间的歧义）。
2. 当封面被识别为 Ultra Dark 时，`uiAccentOnDark` 允许 L 下沉到 0.50–0.58（而非强制 ≥ 0.66），保留原图的夜色感。`globalAccent`、`coverGradientDominant` 同样松开下限。
3. **可读性边界**继续由 WCAG 对比度迭代守护，不靠 L 阈值兜底。
4. 此机制由 ☆ K.7 Artwork Readability Profile 统一计算输入，避免与 BKArtBackground 字面量 Ultra Dark 形成多套并行判定。
...
**实施**：作为 OKLCH 数学层的扩展能力（在 ☆ R.2 的第 1 阶段中提供 `oklchToneLadder(base:level:hueFamily:)` 原语）。
...
| 2 | 极暗封面 accent 被强提亮 | 🟡 中 | `nearMonochromeAccent` 副作用 | ☆ K.2 系统级 Ultra Dark |
...
### ☆ R.2 第 0 阶段清理工作（在 R2 附录 B 基础上补充）
...
R3 在 R1 / R2 报告基础上的关键修订与新增：

**重要修正（覆盖 R1 / R2 既有结论）**：
- ✱ R3 修正 R1 关于"系统大概仍保留小面积高显著色"的猜测：**采样保留 / 派色完全丢失**，因为 `optimizedAccent` 不读 richPalette（J.1）。
- ✱ R3 修正 R2 中"Ultra Dark 仅作用于 Fullscreen 歌词"的笼统结论：**真正承担系统级极暗保护的是 `nearMonochromeAccent`，但副作用是把极暗封面强行提亮**（J.2 + ☆ K.2）。
...
| `myPlayer2/Services/Theme/SemanticPalette.swift` | 16 角色色，optimizedAccent 99+ 行 |
| `myPlayer2/Utilities/ColorMath.swift` | HSL / WCAG 数学原语 |
| `myPlayer2/Utilities/LEDColorResolver.swift` | OKLCH 范例 |
| `myPlayer2/Utilities/ArtworkColorExtractor.swift` | textPalette 494-609 行 |
| `myPlayer2/Services/Artwork/ArtworkAssetStore.swift` | 第二层缓存（无版本号） |
| `myPlayer2/Views/Home/HomeAmbientShapesBackground.swift` | makePalette / ambientBaseColor |
| `myPlayer2/Views/Home/HomeHeroView.swift` | 独立 heroPalette |
| `myPlayer2/Views/Library/PlaylistDetailView.swift` | headerSemanticPalette 入口 |
| `myPlayer2/Views/Lyrics/AMLLWebView.swift` | WebView 容器 |
| `myPlayer2/Services/Lyrics/LyricsWebViewStore.swift` | applyEffectiveTheme 1323 行 |
| `myPlayer2/Views/Fullscreen/FullscreenPlayerView.swift` | makeFullscreenLyricsColorSet / CoverBlurProfile 3772 行 |
| `myPlayer2/Views/Fullscreen/MiniPlayerSpectrumView.swift` | AppKit Spectrum 容器 |
| `myPlayer2/Resources/AMLL/index.html` | Web 层颜色变量与 mix-blend-mode |
...
- [ ] 评估 `headerSemanticPalette` 是否要继续生成 14 个字段
...
**调查完成日期**：2026-05-19
**调查范围**：两轮共 11 个子代理并行，覆盖 6 个独立颜色消费域 + 8 个定点疑点
**输出去向**：作为 OKLCH 迁移所有阶段的设计依据。本报告与代码同步保留；任何与本报告冲突的代码变更视为对原始系统快照的偏离，需在变更说明中标注。
