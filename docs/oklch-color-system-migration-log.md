...
`COLOR_SYSTEM_SELF_CHECK=1` 本地运行：**ALL PASS**。
...
6. 夜间 + nearMono（真灰）：歌词、背景、shapes、moving circle 都中性；shapes 不再淡粉。
7. 夜间 + low-sat chromatic 封面（暖灰、压暗的复古）：Phase 6.2 不再灰白化，保留封面色相。
8. 日间 + 高饱和封面 + 艺术背景：bg 明显比 6.1 更亮（airy）；shapes 跟着亮；active 是深色但 "alive"（不死黑）；translation = inactive；glow 自动是 dark glow；interlude dots 深色；background lyric 深色；MiniPlayer 控件切到 dark foreground。
...
- `ArtworkColorAnalysis`：`salientHighlightPalette` 现在同步输出 `salientHighlightAreaShares`，供歌词 seed 选择使用；nearMono 判定改成“没有任何可信 hue 才 nearMono”，可信来源包含 dominant / topPalette / richPalette / displayPalette / salient / bestTextSource。低饱和彩色封面不再被 strict mono 平均值灰白化。
- `SemanticPaletteFactory.focusScore`：改成 Phase 6.3 主观视觉焦点评分。新 score 使用 OKLab 感知距离、ΔC、ΔL、Δhue、dominant-field confidence、competing high-saturation area、nonlinear area gate；小面积强对比焦点有机会胜出，30% 大色块和普通多色不会乱跳。
- `BKColorEngine`：trusted-hue 判定与 analyzer 对齐；true nearMono 且无 trusted hue 时，对 bgStops / shapePool / dotBase / bgVariants 全部执行 OKLCH chroma crush，SelfCheck 用“伪粉输入 + 真灰 analysis”覆盖防粉路径。
- 夜间艺术背景：常规 dark tier 降亮并提高 chroma floor，暖红 trusted hue 有更保守的 saturation gating，避免暖色封面灰淡；亮封面不再把 BK 背景抬亮；UltraDark shape/circle 更暗。
- 日间艺术背景：light tier 独立为 high-B airy 设计（bgB 0.94…0.985 / fgB 0.88…0.96 / dotB 0.78…0.90），不再像夜间的浅版。
- 日间歌词：light artistic lyric L 提到 active 0.255 / inactive 0.515 / translation 0.520 / line-timing 0.550…0.575，保持 dark foreground 但不死黑，仍全部低于 day background。
...
- Fullscreen MiniPlayer UI：`FullscreenMiniPlayerView` 与 `FullscreenPlayerView` bottom controls 在 `fullscreenArtBackgroundEnabled && colorScheme == .light` 下固定使用 `readabilityProfile.foregroundPrimary`，并让 icon blend mode 走 normal，切歌不再随封面亮度切白。
- ThemeStore 切歌稳定性：新 artwork cache miss / pending full analysis 时保留上一首 palette，不再先写 `.neutralFallback` / default accent，也不发布 quick-only palette；full analysis ready 后一次性 refresh。真正无 artwork 才 fallback。

**SelfCheck / Build**：

- `COLOR_SYSTEM_SELF_CHECK=1`：`Result: ALL PASS`。新增/更新 Phase 6.3 场景覆盖：黑底小黄、蓝底小橙、70% 棕 + 30% 蓝、tiny noise、普通多色、low-sat chromatic 非 nearMono、true grey nearMono、black + salient 非 nearMono、BK true nearMono shapes neutral、夜间 high-C inactive、mid-C passthrough、UltraDark inactive deeper、night circle below lyrics、day bg/lyrics/gap、day dark glow source、MiniPlayer dark profile。
...
1. 黑底小黄、蓝底小橙、极小强对比标题/Logo：歌词 seed 应更稳定选中焦点色。
2. 70% 棕 + 30% 蓝、普通四色封面、噪点封面：仍走 dominant，不乱跳。
3. 低饱和暖灰 / 复古彩色封面：不再 nearMono 灰白化。
4. 真灰封面：歌词、BK、BK1/BK2、floating shapes、moving circle 全部中性，不偏粉。
5. 夜间暖红封面：背景有彩度但不亮；circle 更暗但仍高于底色；UltraDark inactive 更沉。
6. 日间艺术背景：整体明显 airy；歌词、dots、background lyric、glow 都是深色体系；Fullscreen MiniPlayer icon / text 不再变白。
7. 连续切歌：新歌分析 ready 前保留旧颜色，不闪 default blue / neutral fallback。
...
- 日间艺术背景改为独立 light artistic profile：`BKColorEngine` light tier 提升到 `bgB=0.975…0.995`、`fgB=0.930…0.985`、`dotB=0.860…0.950`，并移除 light path 对 `bMid` 的 `0.85` 硬上限。日间背景是高明度、airy、tinted 的底色层，不再是夜间背景的浅版。
- 日间禁用 UltraDark：`SemanticPalette.artisticFullscreenLyricsColorSet`、`FullscreenPlayerView.makeFullscreenLyricsColorSet`、`BKArtBackgroundView` 与 `SkinContext.ThemeTokens.artBackgroundIsUltraDark` 都只在 `colorScheme == .dark` 时传递 UltraDark。light + artistic background 下不再压暗背景、shapes、circle 或歌词。
- 日间歌词 deep but alive：light lyrics L 提升为 active 0.305 / sub-active 0.410 / inactive 0.560 / translation 0.565 / line timing 0.590…0.610；active 仍比 inactive 更深、更清楚，translation 与 inactive 同层，所有 lyric L 仍低于 day background。
- Fullscreen MiniPlayer UI 统一 fixed dark profile：light + artistic background 下 `FullscreenMiniPlayerView`、bottom volume control、playback mode slider、progress/time row 全部走 `readabilityProfile.foregroundPrimary` / normal blend。`MiniPlayerProgressSpectrumRow` 不再被 `enforceBrightForeground` 拉白；`ExpandableVolumeControl` 新增 `forceDarkForegroundProfile`，hover / expanded 状态不再切到另一套颜色。
- 切歌不闪 default 的 view-layer 修复：ThemeStore 的 pending hold 已存在，但 `FullscreenPlayerView.resolveLyricsAnalysis` 在 `paletteMatches == false` 时仍返回 `.neutralFallback`。现在优先使用 current snapshot analysis；snapshot 不 ready 时保留 ThemeStore 上一首 semantic palette，并在 fullscreen artistic lyrics theme 注入处 hold，避免把 neutral/default 色下发到 AMLL。
...
- nearMono 误判的真实原因修正：Phase 6.3 的 trusted-hue 定义保留，本轮修的是 cache-hit / pending 路径丢 analysis 后被 fallback/neutral 吞掉。有色封面在 snapshot 命中时仍能携带 chromatic candidates；真灰仍走 nearMono。
- nearMono shapes 方向保持：不回退到死灰。true nearMono 下继续允许极低 chroma 的黑白适配淡色，但不走伪 hue / fallback pink。
- Emphasis glow 保持 Phase 6.3 正确方向：Swift-owned dark glow contract 未改；本轮未改 `index.html`、未改 generated AMLL bundle。

**SelfCheck / Build**：

- `COLOR_SYSTEM_SELF_CHECK=1`：`Result: ALL PASS`。新增 Phase 6.4 gate 覆盖 light artistic ignores UltraDark、day art background airy high-B bands；Phase 6.3 的 focus seed、nearMono、防粉、dark glow、MiniPlayer profile 回归仍通过。
- `xcodebuild -project kmgccc_player.xcodeproj -scheme kmgccc_player -configuration Debug -destination 'platform=macOS' build`：PASS。
...
Self-check 场景随 Phase 6.1–6.4 持续扩展；以 `COLOR_SYSTEM_SELF_CHECK=1` 输出为准。2026-05-23 本地 Debug 运行：`Result: ALL PASS`，覆盖 focus seed、nearMono trusted hue、BK true nearMono shapes、日夜艺术背景、日间 dark glow、MiniPlayer fixed dark profile、light-mode UltraDark 禁用与 pending palette hold 相关断言。
