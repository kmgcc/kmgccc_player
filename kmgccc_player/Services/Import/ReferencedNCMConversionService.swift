...
final class ReferencedNCMConversionService {
    ...
    func convert(_ file: ImportDiscoveredFile) async throws -> ReferencedNCMConversionOutput {
        ...
        if let active = try await registry.activeRecord(matching: sourceFingerprint) {
            // Reservations left `.pending` by a dead process are retired when
            // the journal loads, so a pending record here belongs to another
            // in-flight conversion of this session; do not race it by
            // starting a second decode.  An output-ready record, on the other
            // hand, is recoverable unless its generated product was removed
            // or corrupted.  In that case retire the stale journal entry and
            // continue through the normal reusable / conversion path so the
            // source can rebuild itself.
            guard active.state == .outputReady else {
                throw ReferencedNCMConversionError.activeReservation(active.id)
            }
            do {
                return try await recover(active, sourceURL: sourceURL, sourceFingerprint: sourceFingerprint)
            } catch let error as ReferencedNCMConversionError {
                switch error {
                case .recoveryOutputMissing, .invalidOutput:
                    try? await registry.markFailed(
                        operationID: active.id,
                        summary: "Generated output is missing or invalid; rebuilding"
                    )
                default:
                    throw error
                }
    ...
    private func recover(
        ...
        if let persisted = record.outputLocator {
            ...
            if !record.sourceMemberships.isEmpty {
                repaired.replacePrimarySourceMemberships(record.sourceMemberships)
    ...
    func restoreCommittedOutputLocator(
        ...
        if !record.sourceMemberships.isEmpty {
            locator.replacePrimarySourceMemberships(record.sourceMemberships)
    ...
    private func outputURL(
        ...
        // If the marker proves that this exact path was generated by the app
        // for this exact source, it is safe to replace a damaged product in
        // place.  An unmarked user file is never deleted; it keeps the
        // existing suffix behaviour and remains protected from overwrite.
        if let generated = NCMGeneratedOutputMarkerStore.record(
            forOutputPath: requestedURL,
            fileManager: fileManager
        ),
           NCMGeneratedOutputMarkerStore.sameExactFingerprint(
            ...
            try? fileManager.removeItem(at: requestedURL)
            return (requestedURL, false)
        }
