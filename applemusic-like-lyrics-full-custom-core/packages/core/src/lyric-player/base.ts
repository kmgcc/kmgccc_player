	...
	implements HasElement, Disposable
{
		...
		for (let i = this.processedLines.length - 1; i >= 0; i--) {
			...
			if (i > 0) {
				...
				if (prevIdx >= 0) {
					...
					prevRawEndTime = this.currentLyricLines[prevIdx]?.endTime;
				}
			}
	...
	async calcLayout(sync = false, force = false) {
		...
		const duetDotRightInset = Math.min(16, Math.max(10, dotMargin * 0.35));
		...
		this.currentLyricLineObjects.forEach((lineObj, i) => {
			...
			if (!setDots && shouldShowDots) {
				...
				if (interlude && isNextDuet) {
					targetX =
						this.size[0] - this.interludeDotsSize[0] - duetDotRightInset;
				}
